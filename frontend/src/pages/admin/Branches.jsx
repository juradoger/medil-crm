// Gestión de sucursales (solo admin) — Branch management (admin only)
...
function BranchModal({ initial, onSave, onClose }) {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (!form.name.trim()) { setError('El nombre es obligatorio — Name is required'); return; }
    setSaving(true);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">
          {initial ? 'Editar Sucursal — Edit Branch' : 'Nueva Sucursal — New Branch'}
        </h2>
        <form onSubmit={submit} className="space-y-4">
          <FormField label="Nombre — Name">
            <input className={inputClass} value={form.name} onChange={e => set('name', e.target.value)} required />
          </FormField>
          <FormField label="Ciudad — City">
            <input className={inputClass} value={form.city} onChange={e => set('city', e.target.value)} />
          </FormField>
          <FormField label="Dirección — Address">
...
export default function Branches() {
  ...
  const columns = [
    ...
    { key: 'city', label: 'Ciudad — City' },
    { key: 'address', label: 'Dirección — Address' },
    { key: 'phone', label: 'Teléfono — Phone' },
    {
      key: 'actions', label: '',
      render: r => (
        <button onClick={() => setModal(r)} className="text-xs text-[#00B4D8] hover:underline">
          Editar — Edit
        </button>
      ),
    },
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#0E4A8A]">Sucursales — Branches</h1>
        <button
      ...
      <DataTable columns={columns} rows={branches} emptyTitle="Sin sucursales — No branches" />
